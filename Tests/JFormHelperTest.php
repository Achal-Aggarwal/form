...
class JFormHelperTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testLoadFieldType()
	{
		...
		$this->assertThat(
			(FormHelper::loadFieldType('modal_foo') instanceof \Joomla\Form\Field\Modal_FooField),
			$this->isTrue(),
			'Line:' . __LINE__ . ' loadFieldType should return the correct custom class.'
		);

		$this->assertThat(
			(FormHelper::loadFieldType('foo.modal_bar') instanceof \Foo\Form\Field\Modal_BarField),
			$this->isTrue(),
			'Line:' . __LINE__ . ' loadFieldType should return the correct custom class.'
		);
	}
